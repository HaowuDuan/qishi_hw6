...
def drop_and_merge_excel(excel_info, merged_info):
    # 获取所有工作表名称
    try:
        # 从merged_info中获取所有工作表名称
        sheet_names = list(merged_info.keys()) if merged_info else ['Sheet1']
        
        # 为每个工作表生成默认配置
        result = []
        for sheet_name in sheet_names:
            result.append({sheet_name: {'labels': [], 'header': [1, 2]}})
...
def get_excel_data(file_path, head=6):
    try:
        ...
        for sheet_name, data in all_sheets_data.items():
            logger.info(f"Processing sheet: {sheet_name}, shape: {data.shape}")
            ...
            # 使用to_string替代to_markdown避免tabulate依赖
            sheet_first_rows = data.head(head).to_string(index=True)
            sheet_info = f"Sheet: {sheet_name}\n前 {head} 行数据为：\n\n{sheet_first_rows}\n\n---"
            prompt_parts.append(sheet_info)
        
        logger.info(f"Extracted data from {len(prompt_parts)} sheets")
        return prompt_parts
    except Exception as e:
        logger.error(f"提取prompt失败：\n{e}", exc_info=True)
        return []
...
def main_unmerge_file(input_file, output_path):
    try:
        logger.info(f"Starting main_unmerge_file with input: {input_file}, output: {output_path}")
        
        # 首先检查原始文件是否已经有正确的表头
        try:
            df_original = pd.read_excel(input_file, sheet_name=0)
            logger.info(f"Original file shape: {df_original.shape}")
            logger.info(f"Original columns: {list(df_original.columns)}")
            
            # 如果原始文件已经有合理的列名，直接使用
            if len(df_original.columns) > 0 and not all(col.startswith('Unnamed') for col in df_original.columns):
                logger.info("Original file has good headers, using directly")
                with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
                    df_original.to_excel(writer, sheet_name='Sheet1', index=False)
                return output_path
        except Exception as e:
            logger.warning(f"Could not read original file directly: {e}")
        
        # 解除合并文件
        unmerged_file = f"unmerged_file_{uuid.uuid4()}.xlsx"  # 使用唯一文件名
        try:
            logger.info("Step 1: Unmerging and filling Excel...")
            _, merged_info = unmerge_and_fill_excel(input_file, unmerged_file)
            logger.info(f"Unmerge completed, merged_info keys: {list(merged_info.keys()) if merged_info else 'None'}")

            logger.info("Step 2: Getting Excel data...")
            sheet_info = get_excel_data(file_path=unmerged_file)  # 获取每个表单前6行数据
            logger.info(f"get_excel_data returned {len(sheet_info) if sheet_info else 0} items")
            
            if sheet_info and len(sheet_info) > 0:  # 检查是否为空列表
                excel_info = '\n'.join(sheet_info)
                logger.info(f"Excel info length: {len(excel_info)}")
            else:
                logger.warning("get_excel_data returned empty or None, using fallback")
                excel_info = "No data available"
                
            logger.info("Step 3: Getting label info...")
            lable_info = drop_and_merge_excel(excel_info=excel_info,
                                              merged_info=merged_info)  # 根据前6行数据，让大模型指出应该删除的说明性文本以及应该合并的多级表头
            logger.info(f'原始结果:\n{lable_info}')
            
            logger.info("Step 4: Parsing JSON...")
